    ...
    if request.method == 'POST':
        ...
        if title and workout_type and duration and calories and workout_date:
            ...
            conn.execute('''
                ...
                (workout_title, workout_type, duration, calories_burned, workout_date, notes)
                VALUES (?, ?, ?, ?, ?, ?)
...
def assign_workout():
    ...
    if request.method == 'POST':
        ...
        if title and workout_type and duration and calories and workout_date:
            ...
            conn.execute('''
                INSERT INTO workouts
                (workout_title, workout_type, duration, calories_burned, workout_date, status)
                VALUES (?, ?, ?, ?, ?, ?)
...
def analytics():
    ...
    workouts_by_status = conn.execute('''
        SELECT status, COUNT(*) as count FROM workouts GROUP BY status
    ''').fetchall()

    weekly_data = conn.execute('''
        SELECT workout_date, COUNT(*) as count,
               SUM(duration) as duration, SUM(calories_burned) as calories
        FROM workouts
    ...
    monthly_data = conn.execute('''
        SELECT strftime('%Y-%m-%d', workout_date) as date,
               COUNT(*) as count, SUM(calories_burned) as calories
        FROM workouts
        WHERE workout_date >= date('now', '-30 days')
        GROUP BY strftime('%Y-%m-%d', workout_date) ORDER BY date
    ''').fetchall()

    best_day = conn.execute('''
        SELECT workout_date, SUM(calories_burned) as total_calories
        FROM workouts GROUP BY workout_date
...
def goals():
    ...
    for goal in all_goals:
        ...
        if current >= goal['target_value'] and goal['status'] != 'Achieved':
            conn.execute("UPDATE goals SET status='Achieved' WHERE id=?", (goal['id'],))
